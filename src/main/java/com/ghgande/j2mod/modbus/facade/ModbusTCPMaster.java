...
import com.ghgande.j2mod.modbus.net.TCPMasterCallback;
...
public class ModbusTCPMaster extends AbstractModbusMaster {
    ...
    public static synchronized void listen(int port, boolean useRtuOverTcp, ModbusListenerCallback callback) throws IOException {
        TCPMasterCallback tcpMasterCallback = (serialId, connection) -> {
            ModbusTCPMaster master = new ModbusTCPMaster(connection.getAddress().getHostAddress());
        ...
        TCPMasterConnection.listen(port, useRtuOverTcp, tcpMasterCallback);
    }
